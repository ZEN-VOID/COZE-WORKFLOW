...
| process_node | `src/graphs/nodes/process_node.py` | agent | 使用大语言模型处理文本 | - | `config/llm/process_node.json` |
...
- 建议通过 `config/graph/main.yaml` 统一维护节点与配置文件映射
- 提交前必须运行 `python scripts/validate_config.py`
...
```bash
# 运行完整工作流
bash scripts/local_run.sh -m flow -i '{"input_text":"测试输入"}'
