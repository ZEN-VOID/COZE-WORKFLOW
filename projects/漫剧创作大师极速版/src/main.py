...
setup_logging(
    log_file=settings.logging.log_file or LOG_FILE,
    max_bytes=settings.logging.max_bytes,
    backup_count=settings.logging.backup_count,
    log_level=settings.logging.level or LOG_LEVEL,
    use_json_format=settings.logging.json_format,
...
def parse_args():
    ...
    parser.add_argument("-p", type=int, default=settings.http.port, help="HTTP server port")
...
def parse_input(input_str: str) -> Dict[str, Any]:
    ...
    if not input_str:
        return {"input_text": settings.workflow.default_input_text}
...
def start_http_server(port):
    ...
    reload = settings.http.reload or graph_helper.is_dev_env()
